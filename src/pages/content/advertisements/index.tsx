...
const advertisementTypes: { value: AdvertisementType; label: string }[] = [
  ...
  { value: 'middle_home_banner', label: 'Middle home banner' },
  { value: 'bottom_cta_banner', label: 'Bottom CTA banner' },
...
export function AdvertisementsPage() {
  ...
  const openEdit = async (item: Advertisement) => {
    ...
    try {
      ...
      setErrors(['تعذر تحميل تفاصيل الإعلان، يمكنك تعديل البيانات الظاهرة من القائمة.'])
    }
  }
  ...
  const validateForm = () => {
    ...
    if (!form.type) nextErrors.push('نوع الإعلان مطلوب.')
    if (!form.starts_at) nextErrors.push('تاريخ بداية الظهور مطلوب.')
    if (!form.ends_at) nextErrors.push('تاريخ نهاية الظهور مطلوب.')
    if (form.is_active === '') nextErrors.push('الحالة مطلوبة.')
    if (!imageFile && !imagePreview) nextErrors.push('صورة الإعلان مطلوبة.')
    return nextErrors
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrors(errorMessages(err, 'حدث خطأ أثناء حفظ الإعلان.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const columns = useMemo<ColumnDef<Advertisement>[]>(() => [
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="line-clamp-1 text-xs text-zinc-500">{firstText(row.original.subtitle, row.original.subtitle_ar) || 'بدون وصف'}</p>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      header: 'الحالة',
      cell: ({ row }) => <StatusBadge active={isActive(row.original.is_active ?? row.original.status)} />,
    },
    {
      id: 'actions',
      header: '',
  ...
  return (
    <div className="space-y-8" dir="rtl">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">إدارة الإعلانات</h1>
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">إضافة وتعديل إعلانات التطبيق ومواقع ظهورها.</p>
        </div>
      ...
      <Card className="overflow-hidden border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-[#0f0f11]">
        <div className="grid gap-3 border-b border-zinc-200 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-900/50 md:grid-cols-[1fr_220px_180px]">
          <div className="relative">
            ...
            <Input
              ...
              className="bg-white pr-10 dark:bg-zinc-950"
            />
          </div>
          <Select value={filters.type} onValueChange={(value) => setFilters((previous) => ({ ...previous, type: value }))}>
            ...
            <SelectContent>
              <SelectItem value={allValue}>كل الأنواع</SelectItem>
          ...
          <Select value={filters.status} onValueChange={(value) => setFilters((previous) => ({ ...previous, status: value }))}>
            ...
            <SelectContent>
              <SelectItem value={allValue}>كل الحالات</SelectItem>
        ...
        <DataTable
          ...
          emptyMessage="لا توجد إعلانات مضافة حتى الآن."
        />
      </Card>

      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="left" className="w-full overflow-y-auto dark:border-zinc-800 dark:bg-zinc-950 sm:max-w-3xl" dir="rtl">
          <SheetHeader className="mb-6">
            <SheetTitle>{editing ? 'تعديل الإعلان' : 'إضافة إعلان جديد'}</SheetTitle>
          </SheetHeader>
          ...
          <div className="grid gap-5">
            <div className="grid gap-4 md:grid-cols-2">
              <SelectInput label="نوع الإعلان" value={form.type} onChange={(value) => setForm((previous) => ({ ...previous, type: value as AdvertisementType }))} options={advertisementTypes} required />
              <SelectInput label="نوع الهدف" value={form.target_type} onChange={(value) => setTargetType(value as TargetType)} options={targetTypes} />
            </div>

            {form.target_type === 'external_url' && (
              <TextInput label="رابط خارجي" value={form.external_url} onChange={(value) => setForm((previous) => ({ ...previous, external_url: value }))} dir="ltr" />
            )}

            <div className="grid gap-4 md:grid-cols-2">
              <DateInput label="تاريخ بداية الظهور" value={form.starts_at} onChange={(value) => setForm((previous) => ({ ...previous, starts_at: value }))} required />
              <DateInput label="تاريخ نهاية الظهور" value={form.ends_at} onChange={(value) => setForm((previous) => ({ ...previous, ends_at: value }))} required />
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <SelectInput label="الحالة" value={form.is_active} onChange={(value) => setForm((previous) => ({ ...previous, is_active: value }))} options={[{ value: '1', label: 'Active' }, { value: '0', label: 'Inactive' }]} required />
            </div>

            <div className="grid gap-4 md:grid-cols-[1fr_220px]">
              <div className="space-y-2">
                <Label>صورة الإعلان <span className="text-red-500">*</span></Label>
                <Input type="file" accept="image/*" onChange={(event) => setImageFile(event.target.files?.[0] || null)} />
                <p className="text-xs text-zinc-500">الصورة مطلوبة. عند التعديل يمكنك ترك الصورة الحالية أو اختيار صورة جديدة.</p>
              </div>
              <div className="flex aspect-video items-center justify-center overflow-hidden rounded-md border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
                {imagePreview ? (
                  <img src={imagePreview} alt="معاينة صورة الإعلان" className="h-full w-full object-cover" />
                ) : (
          ...
          <div className="mt-8 flex gap-3">
            <Button className="flex-1 gap-2 bg-teal-600 text-white hover:bg-teal-700" onClick={handleSave} disabled={saving}>
              ...
              {editing ? 'حفظ التعديلات' : 'إضافة الإعلان'}
            </Button>
      ...
      <Sheet open={detailsOpen} onOpenChange={setDetailsOpen}>
        <SheetContent side="left" className="w-full overflow-y-auto dark:border-zinc-800 dark:bg-zinc-950 sm:max-w-xl" dir="rtl">
          <SheetHeader className="mb-6">
            <SheetTitle>تفاصيل الإعلان</SheetTitle>
          </SheetHeader>
          {details && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">{firstText(details.subtitle, details.subtitle_ar) || 'بدون وصف'}</p>
              </div>
              <div className="grid gap-3 text-sm">
                <DetailRow label="نوع الإعلان" value={typeLabel(details.type)} />
                <DetailRow label="نوع الهدف" value={targetLabel(details.target_type)} />
                <DetailRow label="رقم الهدف" value={cleanText(details.target_id) || '-'} />
                <DetailRow label="الرابط الخارجي" value={cleanText(details.external_url) || '-'} ltr />
                <DetailRow label="نص الزر" value={firstText(details.button_text, details.button_text_ar) || '-'} />
                <DetailRow label="ترتيب الظهور" value={cleanText(details.sort_order) || '-'} />
                <DetailRow label="بداية الظهور" value={cleanText(details.starts_at) || '-'} ltr />
                <DetailRow label="نهاية الظهور" value={cleanText(details.ends_at) || '-'} ltr />
                <div className="flex items-center justify-between border-b border-zinc-100 pb-3 dark:border-zinc-800">
                  <span className="text-zinc-500">الحالة</span>
                  <StatusBadge active={isActive(details.is_active ?? details.status)} />
                </div>
              </div>
            </div>
          )}
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(open) => !open && setToDelete(null)}>
        <AlertDialogContent className="dark:border-zinc-800 dark:bg-[#0f0f11]" dir="rtl">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-right">تأكيد الحذف</AlertDialogTitle>
            <AlertDialogDescription className="text-right">
              هل تريد حذف إعلان "{displayTitle(toDelete)}"؟
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-4 flex-row-reverse gap-2 sm:flex-row-reverse sm:justify-start">
            <AlertDialogCancel className="mt-0">إلغاء</AlertDialogCancel>
            <AlertDialogAction className="bg-red-600 text-white hover:bg-red-700" onClick={handleDelete}>حذف نهائي</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <Input
          ...
          dir="ltr"
          className="text-left pr-11 [color-scheme:light] dark:[color-scheme:dark] [&::-webkit-calendar-picker-indicator]:opacity-0"
        />
        <button
          type="button"
          onClick={openPicker}
          className="absolute right-3 top-1/2 flex h-5 w-5 -translate-y-1/2 items-center justify-center text-zinc-600 transition-colors hover:text-teal-600 dark:text-zinc-300 dark:hover:text-teal-400"
          aria-label={label}
        >
...
function typeLabel(value?: string) {
  return advertisementTypes.find((type) => type.value === value)?.label || cleanText(value) || '-'
}

function targetLabel(value?: string) {
  return targetTypes.find((type) => type.value === value)?.label || cleanText(value) || 'بدون هدف'
}

function isAdvertisementType(value: unknown): value is AdvertisementType {
  return advertisementTypes.some((type) => type.value === value)
}

function isTargetType(value: unknown): value is TargetType {
  return targetTypes.some((type) => type.value === value)
}
