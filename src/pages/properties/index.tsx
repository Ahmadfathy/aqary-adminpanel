...
export function PropertiesPage() {
  ...
  const getPageTitle = () => {
    if (purpose === 'sale') return tc('property.forSale')
    if (purpose === 'rent') return tc('property.forRent')
    return tc('property.all')
  }
  ...
  const handleReject = async (id: number) => {
    try {
      await AdminPropertiesAPI.rejectProperty(id, tc('property.rejectedByAdmin'))
      fetchProperties()
    } catch (error) {
  ...
  const getStatusBadge = (status: string | number | boolean) => {
    if (status === 'approved' || status === 1 || status === true || status === 'active') {
      return <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium text-emerald-700 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-500/10"><CheckCircle className="w-3.5 h-3.5" /> {tc('property.approvedStatus')}</span>
    }
    if (status === 'rejected' || status === 'suspended' || status === 0 || status === false) {
      return <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium text-red-700 bg-red-50 dark:text-red-400 dark:bg-red-500/10"><Ban className="w-3.5 h-3.5" /> {tc('property.suspendedStatus')}</span>
    }
    return <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium text-amber-700 bg-amber-50 dark:text-amber-400 dark:bg-amber-500/10"><Loader2 className="w-3.5 h-3.5" /> {tc('property.pendingStatus')}</span>
  }
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          const title = property.title || property.name_ar || property.name || tc('property.detail.noTitle')
          return (
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                  {tc('property.refNo')} {property.reference_number || property.id}
                </p>
              </div>
            </div>
          )
      ...
      },
      {
        ...
        header: tc('property.location'),
        cell: ({ row }) => {
          const property = row.original
          return (
            <div className="flex items-center gap-1.5 text-sm text-zinc-600 dark:text-zinc-400">
              <MapPin className="w-3.5 h-3.5 text-zinc-400" />
              <span className="line-clamp-1 max-w-[150px]">{property.address || property.city?.name_ar || property.city?.name || property.city || property.location || tc('common.unspecified')}</span>
            </div>
          )
        },
      },
      {
        ...
        header: tc('property.publisher'),
        cell: ({ row }) => {
          const property = row.original
          return (
            <div className="flex flex-col">
              <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{property.owner?.name || property.user?.name || property.owner_name || tc('common.unknown')}</span>
              <span className="text-xs text-zinc-500 dark:text-zinc-400">{(property.owner?.user_type || property.user?.user_type) === 'office' ? tc('offices.type') : tc('property.ownerClient')}</span>
            </div>
          )
        },
      },
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 dark:bg-zinc-900 dark:border-zinc-800">
                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => navigate(`/real-estate/${property.id}`)}>
                  <Eye className="w-4 h-4" /> {tc('btn.viewDetails')}
                </DropdownMenuItem>
                ...
                {(property.status === 'pending' || property.status === 'suspended' || property.status === 0 || property.status === false || property.status === 'rejected') && (
                  <DropdownMenuItem className="gap-2 cursor-pointer focus:bg-emerald-50 dark:focus:bg-emerald-500/10 focus:text-emerald-600" onClick={() => handleApprove(property.id)}>
                    <CheckCircle className="w-4 h-4 text-emerald-500" /> <span className="text-emerald-600">{tc('property.approveBtn')}</span>
                  </DropdownMenuItem>
                )}
                ...
                {(property.status === 'approved' || property.status === 1 || property.status === true || property.status === 'active' || property.status === 'pending') && (
                  <DropdownMenuItem className="gap-2 cursor-pointer focus:bg-orange-50 dark:focus:bg-orange-500/10 focus:text-orange-600" onClick={() => handleReject(property.id)}>
                    <Ban className="w-4 h-4 text-orange-500" /> <span className="text-orange-600">{tc('property.rejectBtn')}</span>
                  </DropdownMenuItem>
                )}
      ...
      <Card className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0f0f11] shadow-sm overflow-hidden">
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="relative max-w-md">
            ...
            <Input
              type="text"
              placeholder={tc('property.searchPlaceholder')}
              value={search}
        ...
        <DataTable 
          ...
          emptyMessage={tc('property.noFound')}
        />
      </Card>
      ...
      <AlertDialog open={propertyToDelete !== null} onOpenChange={(open) => !open && setPropertyToDelete(null)}>
        <AlertDialogContent className="dark:bg-[#0f0f11] dark:border-zinc-800">
          ...
          <AlertDialogFooter className="flex-row-reverse sm:flex-row-reverse sm:justify-start gap-2 mt-4">
            ...
            <AlertDialogAction className="bg-red-600 hover:bg-red-700 text-white" onClick={() => propertyToDelete && handleDelete(propertyToDelete)}>{tc('btn.deletePermanent')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
