...
import { Search, Plus, MoreVertical, Edit, Trash2, CheckCircle2, XCircle, Globe } from 'lucide-react'
...
export function CountriesPage() {
  ...
  const fetchCountries = async () => {
    setIsLoading(true)
    try {
      const response = await AdminLocationsAPI.getCountries({ search }).catch(() => ({ data: { data: [] } }))
      let data = response?.data?.data?.data || response?.data?.data || response?.data || [];
      if (!Array.isArray(data)) {
         data = data?.items || data?.countries || data?.data || [];
      }
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      {
        accessorKey: "name",
        header: tc('settings.countries.col'),
        cell: ({ row }) => {
          const country = row.original
          return (
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-lg">
                {country.flag || '🌍'}
              </div>
              <div>
                <p className="font-medium text-zinc-900 dark:text-white">{country.name_ar || country.name || tc('common.noName')}</p>
                <p className="text-xs text-zinc-500">{country.name_en || ''}</p>
              </div>
            </div>
          )
        },
      ...
      {
        accessorKey: "code",
        header: tc('settings.countries.code'),
        cell: ({ row }) => <span dir="ltr" className="font-medium">{row.original.code || '—'}</span>,
      },
      {
        accessorKey: "status",
          ...
          return (
            <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${
              ...
              {isActive ? (
                ...
                <><XCircle className="w-3.5 h-3.5" /> {tc('status.inactive')}</>
              )}
            </span>
          )
        },
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="text-3xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 to-zinc-500 dark:from-white dark:to-zinc-400">
          {tc('settings.countries.title')}
        </h1>
        <div className="flex items-center gap-3">
          <Button className="bg-teal-600 hover:bg-teal-700 text-white gap-2">
            <Plus className="w-4 h-4" />
            {tc('settings.countries.add')}
          </Button>
        </div>
      </div>
      ...
      <Card className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0f0f11] shadow-sm overflow-hidden">
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="relative max-w-md">
            ...
            <Input
              type="text"
              placeholder={tc('settings.countries.search')}
              value={search}
        ...
        <DataTable
          ...
          data={countries}
          isLoading={isLoading}
          emptyIcon={<Globe className="h-5 w-5 text-zinc-400" />}
          emptyMessage={tc('settings.countries.noFound')}
        />
      </Card>
    </div>
  )
