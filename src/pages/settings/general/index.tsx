...
export function SettingsGeneralPage() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        setErrors(errorMessages(err, 'فشل في جلب الإعدادات.'))
      } finally {
        setIsFetching(false)
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrors(errorMessages(err, 'فشل في حفظ الإعدادات.'))
    } finally {
      setIsSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 to-zinc-500 dark:from-white dark:to-zinc-400">إعدادات الأدمن</h1>
          <p className="text-sm text-zinc-500 mt-1">تحكم عام في الإعلانات والاشتراكات وإصدار التطبيق.</p>
        </div>
      ...
      {saved && <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-sm">تم حفظ الإعدادات بنجاح.</div>}
      ...
      <Card className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0f0f11] shadow-sm">
        <CardContent className="p-6 space-y-6">
          <div className="flex items-center gap-3 pb-2 border-b border-zinc-200 dark:border-zinc-800">
            ...
            <div>
              <h2 className="font-semibold text-zinc-900 dark:text-white">الإعدادات العامة</h2>
              <p className="text-sm text-zinc-500">لا يوجد إنشاء للإعدادات، فقط عرض وتعديل.</p>
            </div>
          </div>

          <label className="flex items-center justify-between gap-4 rounded-lg border border-zinc-200 dark:border-zinc-800 p-4">
            <div>
              <p className="font-medium text-zinc-900 dark:text-white">تفعيل إعلانات التطبيق</p>
              <p className="text-sm text-zinc-500">يتحكم في ظهور الإعلانات داخل التطبيق.</p>
            </div>
          ...
          <label className="flex items-center justify-between gap-4 rounded-lg border border-zinc-200 dark:border-zinc-800 p-4">
            <div>
              <p className="font-medium text-zinc-900 dark:text-white">تفعيل اشتراكات المكاتب</p>
              <p className="text-sm text-zinc-500">عند التعطيل لا يظهر زر الاشتراك للمكاتب.</p>
            </div>
