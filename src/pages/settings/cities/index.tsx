...
export function CitiesPage() {
  ...
  const fetchCities = async () => {
    setIsLoading(true)
    try {
      const response = await AdminLocationsAPI.getCities({ search }).catch(() => ({ data: { data: [] } }))
      let data = response?.data?.data?.data || response?.data?.data || response?.data || [];
      if (!Array.isArray(data)) {
        data = data?.items || data?.cities || data?.data || [];
      }
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-zinc-900 dark:text-white">{city.name_ar || city.name || 'بدون إسم'}</p>
