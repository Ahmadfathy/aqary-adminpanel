...
export function GovernoratesPage() {
  ...
  const handleSave = async () => {
    if (!form.name_ar.trim()) {
      setErrors([tc('settings.governorates.nameArRequired')])
      return
    }
    setSaving(true)
    setErrors([])
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => [
    {
      accessorKey: 'name_ar',
      header: tc('settings.governorates.col'),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: tc('settings.governorates.colCountry'),
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="text-3xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 to-zinc-500 dark:from-white dark:to-zinc-400">{tc('settings.governorates.title')}</h1>
        <Button className="bg-teal-600 hover:bg-teal-700 text-white gap-2" onClick={openAdd}><Plus className="w-4 h-4" />{tc('settings.governorates.add')}</Button>
      </div>
      ...
      <Card className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0f0f11] shadow-sm overflow-hidden">
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 grid gap-3 md:grid-cols-[1fr_160px_160px]">
          <div className="relative">
            ...
            <Input placeholder={tc('settings.governorates.search')} value={filters.search} onChange={e => setFilters(p => ({ ...p, search: e.target.value }))} className="pr-10 bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800" />
          </div>
          ...
          <Select value={filters.status} onValueChange={v => setFilters(p => ({ ...p, status: v }))} dir="rtl">
            ...
            <SelectContent><SelectItem value="all">{tc('status.allStatuses')}</SelectItem><SelectItem value="1">{tc('status.active')}</SelectItem><SelectItem value="0">{tc('status.inactive')}</SelectItem></SelectContent>
          </Select>
        </div>
        <DataTable columns={columns} data={governorates} isLoading={isLoading} emptyIcon={<MapPin className="h-5 w-5 text-zinc-400" />} emptyMessage={tc('settings.governorates.noFound')} />
      </Card>

      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="left" className="w-full sm:max-w-md dark:bg-zinc-950 dark:border-zinc-800" dir="rtl">
          <SheetHeader className="mb-6"><SheetTitle>{editing ? tc('settings.governorates.editTitle') : tc('settings.governorates.addTitle')}</SheetTitle></SheetHeader>
          ...
          <div className="space-y-4">
            <div className="space-y-2"><Label>{tc('common.nameAr')} <span className="text-red-500">*</span></Label><Input value={form.name_ar} onChange={e => setForm(p => ({ ...p, name_ar: e.target.value }))} placeholder={tc('settings.governorates.nameArPlaceholder')} /></div>
            <div className="space-y-2"><Label>{tc('common.nameEn')}</Label><Input value={form.name_en} onChange={e => setForm(p => ({ ...p, name_en: e.target.value }))} placeholder="e.g. Baghdad" dir="ltr" /></div>
          ...
          <div className="flex gap-3 mt-8">
            <Button className="flex-1 bg-teal-600 hover:bg-teal-700 text-white gap-2" onClick={handleSave} disabled={saving}>{saving && <Loader2 className="w-4 h-4 animate-spin" />}{editing ? tc('btn.saveChanges') : tc('settings.governorates.addBtn')}</Button>
      ...
      <AlertDialog open={!!toDelete} onOpenChange={open => !open && setToDelete(null)}>
        <AlertDialogContent className="dark:bg-[#0f0f11] dark:border-zinc-800">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-right">{tc('common.deleteConfirmTitle')}</AlertDialogTitle>
            <AlertDialogDescription className="text-right">{tc('settings.governorates.deleteConfirm', { name: entityName(toDelete) })}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-row-reverse sm:flex-row-reverse sm:justify-start gap-2 mt-4">
            <AlertDialogCancel className="mt-0">{tc('btn.cancel')}</AlertDialogCancel>
