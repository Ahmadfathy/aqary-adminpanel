...
export function NeighborhoodsPage() {
  ...
  useEffect(() => {
    const t = setTimeout(fetchNeighborhoods, 500)
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-zinc-900 dark:text-white">{nb.name_ar || nb.name || 'بدون اسم'}</p>
              <p className="text-xs text-zinc-500">{nb.name_en || ''}</p>
            </div>
          </div>
        )
      },
    },
    {
      ...
      cell: ({ row }) => {
        const nb = row.original
        const cityName = typeof nb.city === 'object' ? nb.city?.name_ar || nb.city?.name : nb.city || nb.city_name
    ...
    <div className="space-y-8">
      ...
      <Card className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0f0f11] shadow-sm overflow-hidden">
        ...
        <DataTable
          ...
          emptyMessage="لا توجد أحياء مضافة حتى الآن."
        />
      </Card>
    </div>
  )
