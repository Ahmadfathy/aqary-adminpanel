...
export function RegionsPage() {
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      {
        accessorKey: "name",
        header: tc('settings.regions.col'),
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-zinc-900 dark:text-white">{region.name_ar || region.name || tc('common.noName')}</p>
      ...
      {
        accessorKey: "city",
        header: tc('settings.regions.colCity'),
        cell: ({ row }) => {
          const region = row.original
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="text-3xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 to-zinc-500 dark:from-white dark:to-zinc-400">
          {tc('settings.regions.title')}
        </h1>
        <div className="flex items-center gap-3">
          <Button className="bg-teal-600 hover:bg-teal-700 text-white gap-2">
            <Plus className="w-4 h-4" />
            {tc('settings.regions.add')}
          </Button>
        </div>
      </div>
      ...
      <Card className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0f0f11] shadow-sm overflow-hidden">
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="relative max-w-md">
            ...
            <Input
              type="text"
              placeholder={tc('settings.regions.search')}
              value={search}
        ...
        <DataTable
          ...
          emptyMessage={tc('settings.regions.noFound')}
        />
      </Card>
    </div>
  )
